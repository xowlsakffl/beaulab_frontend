...
export const Modal: React.FC<ModalProps> = ({
  ...
  return (
    <div className="modal fixed inset-0 z-99999 flex items-center justify-center overflow-y-auto">
      {!isFullscreen && (
        <div className="fixed inset-0 h-full w-full bg-gray-900/20 backdrop-blur-[10px]" onClick={onClose}></div>
      )}
...
export const ModalPanel: React.FC<ModalSectionProps> = ({ children, className = "" }) => {
  return (
    <div className={`rounded-3xl bg-white p-5 shadow-2xl shadow-slate-900/10 sm:p-6 ${className}`}>{children}</div>
  );
};
