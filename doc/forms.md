...
- 검색형 option hook은 `useDebouncedRemoteOptions`를 사용해 debounce, TTL cache, 늦게 도착한 응답 무시 규칙을 공유한다.
- 검색형 option의 `loadOptions(query, signal)`은 signal을 API에 전달한다. 캐시는 `createTimedCache`로 등록해 저장·세션 변경 시 무효화한다.
- `MediaUploader`의 공개 props는 유지한다. 내부에서 파일 카드, 목록 조립(`MediaUploaderLists`), 공통 드래그 정렬(`SortableMediaList`), object URL/순서 helper를 분리한다. 기존 파일·신규 파일·혼합 목록에 정렬 이벤트를 복사하지 않는다.
- 미리보기 object URL은 effect에서 생성하고 썸네일과 확대보기에서 공유한다. 제거한 파일과 unmount된 목록의 URL은 해제하며 render 중에는 생성하지 않는다.
