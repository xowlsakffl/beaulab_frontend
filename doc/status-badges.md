...
- 라벨/색상은 `apps/staff-web/lib/common/review-status.ts`를 우선 사용한다.
...
| `FORCED_STOPPED` 또는 강제중지 계열 | 강제중지 | 주황/빨강 계열 중 도메인 기준 |

버튼:

- 라벨/색상은 `apps/staff-web/lib/common/status-labels.ts`를 우선 사용한다.
- 현재 정상 상태면 `강제중지`
- 현재 강제중지 상태면 `정상노출`
...
라벨/색상은 `apps/staff-web/lib/common/status-labels.ts`를 우선 사용한다. 단, 도메인 문구가 다르면 `privateLabel`처럼 옵션으로 조정한다. 예: 이벤트는 `비공개`, 동영상은 `미공개`.
...
신고상태 뱃지는 `ReportStatusBadge` 또는 `reportStatusBadgeColor`를 우선 사용한다. 동영상처럼 도메인별 워딩이 다른 경우에도 같은 의미의 색상 기준은 맞춘다.
