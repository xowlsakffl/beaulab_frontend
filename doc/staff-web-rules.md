...
- 공지사항: 제목·내용 / 채널·공개여부·상단공지·첨부파일. 상세 오른쪽 하단은 공통 운영 히스토리 카드.
...
- 병의원 메뉴의 회원 이하 항목은 `others`에 배치하고 도메인 메뉴와 실선으로 구분한다. 별도 공통메뉴 제목은 표시하지 않는다.
...
- 신고상태의 `신고접수` 배지는 연한 노랑 배경과 진한 황갈색 글씨를 사용한다.
